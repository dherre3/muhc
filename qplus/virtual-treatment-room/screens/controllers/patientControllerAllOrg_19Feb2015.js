...
myApp.controller('screenController', ['$scope','$modal','$http','$firebaseArray','$interval','$location', function($scope,$modal,$http,$firebaseArray,$interval,$location) {
    ...
    $scope.AriaVenueId = searchObject.location; // get the location from the url
    console.log($scope.AriaVenueId);

    // Get the information for the url-specified location
    $http({
	url: "http://172.26.66.41/devDocuments/screens/php/getLocationInfo.php",
	method: "GET",
	params: {IntermediateVenue: $scope.AriaVenueId} 
    }).success(function (data) {
        $scope.PatientDestination = data.ScreenDisplayName;
        console.log(data);
    });
    ...
    var FirebaseUrl = "https://boiling-fire-1762.firebaseio.com/RC/" + today;
    ...
    //=========================================================================
    // Get the list of resources scheduled for the rooms associated with the given location
    //=========================================================================
    var venueForResources = $scope.AriaVenueId;
    if(   $scope.AriaVenueId == "TX AREA B"
       || $scope.AriaVenueId == "TX AREA C"
       || $scope.AriaVenueId == "TX AREA D"
       || $scope.AriaVenueId == "TX AREA E"
       || $scope.AriaVenueId == "TX AREA F"
       || $scope.AriaVenueId == "TX AREA G"
       || $scope.AriaVenueId == "TX AREA H"
       || $scope.AriaVenueId == "pharmacy"
       || $scope.AriaVenueId == "PodA"
       || $scope.AriaVenueId == "PodB"
       || $scope.AriaVenueId == "PodC"
       || $scope.AriaVenueId == "PodD"
      ){
      venueForResources = "TX AREA A";
    }
    
    function load_resources(){
      $http({
	url: "http://172.26.66.41/devDocuments/screens/php/getScheduledResources.php",
	method: "GET",
	params: {IntermediateVenue: venueForResources} 
      }).success(function (data) {
        $scope.userResources=data;
      });
    };
    ...
    function load_patients(){
      $http.get("http://172.26.66.41/devDocuments/screens/php/getCheckinsAll.php").success(function(data){
        $scope.checkins=data;
      });
    };
    ...
    $scope.callPatient= function(patient){
      ...
      //var child_id = "test";    
      //var child_id = patient.PatientId;    

      //patient.LastName = patient.SSN + "* **** " + patient.DAYOFBIRTH + "**";
      //patient.LastName = "(RAMQ: patient.SSN" + "* **** " + patient.DAYOFBIRTH + "**)";
      //patient.LastName = "(RAMQ: " + patient.SSN + "* **** " + patient.DAYOFBIRTH + "**)";
      patient.LastName = patient.SSN + " (RAMQ: " + patient.SSN + "* **** " + patient.DAYOFBIRTH + "**)";
      ...
      $scope.SMS_message = "Cedars Cancer Centre: " + patient.FirstName + " " + patient.LastName + ", please go to " + $scope.PatientDestination + " for your appointment. You will be seen shortly. Thank you.";
...
//      		$scope.debug_messages = $scope.debug_messages + " SMS response: " + $scope.SMS_response;
//	}
//      );
	...
	$scope.debug_messages = $scope.debug_messages + " --- arriving patient with index: " + $scope.screenRows.$indexFor(patient.PatientSer);
      ...
      // Check the patient into the current location in Aria or Mysql/Medivisit, as appropriate
      if(patient.CheckInSystem == "Aria"){
        $http({
	  url: "http://172.26.66.41/devDocuments/screens/php/checkInPatient.php",
	  method: "GET",
	  params: {CheckinVenue: $scope.AriaVenueId, 
		   ScheduledActivitySer: patient.ScheduledActivitySer, 
		   location: $scope.AriaVenueId 
		  } 
        }); 
      }

      if(patient.CheckInSystem == "Medivisit"){
        console.log('Check in medivisit patient');
        console.log('AppointmentSerNum',patient.ScheduledActivitySer);
        console.log('CheckinVenueName',$scope.AriaVenueId);

        $http({
	  url: "http://172.26.66.41/devDocuments/screens/php/checkInPatientMV.php",
	  method: "GET",
	  params: {CheckinVenue: $scope.AriaVenueId, 
		   ScheduledActivitySer: patient.ScheduledActivitySer, 
		  } 
    ...
    //=========================================================================
    // Function to discharge a patient - this option will only be available for 
    // Medivisit patients
	...
	params: {CheckinVenue: $scope.AriaVenueId, 
      ...
      // results to be provided in a modal
...
myApp.controller('ModalDemoCtrl', function ($scope, $modal, $log, $http) {
  ...
  $http.get("http://172.26.66.41/devDocuments/screens/php/getResourcesAll.php").success(function (data) {
